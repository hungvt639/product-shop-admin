...
const OrderComponent = () => {
    const { orders, docs, pages, setPages, changeData, setFilter, filter } =
        useOrder();
        ...
        <div className="_order p-5 overflow-auto h-full">
            ...
            <div className="flex">
                ...
                {st.map((s, i) => (
                    <span
                        onClick={() =>
                            setFilter({ ...filter, status: i.toString() })
                        }
                        className={`px-5 py-1 rounded cursor-pointer mr-2 border${
                            filter.status === i.toString()
            ...
            <div>
                {orders.map((order, i) => (
                    <div
                        className="flex flex-col bg-gray-100 mt-3 px-5 py-3"
                        key={order._id}
                    >
                        ...
                        <div className="flex justify-between">
                            <div>
                                <Tag color={getColor(order.status)}>
                                    {st[order.status]}
                                </Tag>
                            </div>
                            <Form
