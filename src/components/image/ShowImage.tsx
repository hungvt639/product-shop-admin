...
import "./style.scss";
import { MdRotateLeft, MdRotateRight, MdZoomIn, MdZoomOut, MdClose } from "react-icons/md";
type ShowImageProps = {
  src: string;
  alt: string;
  show: boolean;
  ...
  return (
    <div className={show ? "modal-img modal-img-view" : "modal-img modal-img-not-view"}>
      ...
      <div className="btn-action-img">
        ...
        <span onClick={() => setDataScale(-1)} className={scale === 0 ? "btn-action-img-disable" : ""}>
          <MdZoomIn />
        </span>
        <span onClick={() => setDataScale(+1)} className={scale === scales.length - 1 ? "btn-action-img-disable" : ""}>
          <MdZoomOut />
        </span>
