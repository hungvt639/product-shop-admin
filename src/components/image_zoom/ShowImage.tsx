...
const ShowImage = (props: ShowImageProps) => {
  ...
  const changePosition = useCallback(
    (e: any) => {
      ...
      if (offset) {
        ...
        if (top + rectangle.height > offset.height) top = offset.height - rectangle.height;
        if (top < 0) top = 0;

        let left = e.clientX - offset.left - rectangle.width / 2;
        if (left + rectangle.width > offset.width) left = offset.width - rectangle.width;
        if (left < 0) left = 0;
        setPosition({ left: left, top: top });
        setBgrPosition("-" + left * rectangle.scale + "px -" + top * rectangle.scale + "px");
      }
    },
  ...
  return (
    <div className={show ? "modal-img modal-img-view" : "modal-img modal-img-not-view"}>
      <div onClick={onClose} className="close-modal-img"></div>
      <div className="show-modal-img">
        <div>
          <div onMouseMove={changePosition} className="img-zoom-container">
            <img ref={imgRef} src={src} alt={alt} width={imgW}></img>
            <div
              style={{
                ...position,
                ...rectangle,
